    ...
    def __init__(self, in_channels=1, out_channels=1, num_iterations=[1, 1, 1, 1, 1]):
        ...
        for j in range(len(num_iterations) - 1):
            self.transpose_layers.append(
                nn.ConvTranspose2d(
                    in_channels=in_channels*(j+2), 
                    out_channels=out_channels*(j+1),
                    kernel_size=3, 
                    stride=2, 
                    padding=0, 
                    bias=False
        ...
        for layer_index, num_iterations_layer in enumerate(num_iterations):
            for _ in range(num_iterations_layer):
                layers.append(Conv_Dyn(
                    in_channels=in_channels*(layer_index+1), 
                    out_channels=out_channels*(layer_index+1),
                    resolution=self.resolutions[layer_index]
                ))
            self.conv_layers.append(nn.Sequential(*layers))

            if layer_index < len(num_iterations) - 1:
                layers = [MgRestriction(
                    in_channels=in_channels*(layer_index+1), 
                    out_channels=out_channels*(layer_index+2)
                    )]
...
class MgNO(nn.Module):
    def __init__(self,
        ...
        self.layers = layers
        self.lifting_1 = nn.Conv2d(1, features, kernel_size=1, bias=False)
        self.lifting_2 = nn.Conv2d(2, features, kernel_size=1)

        self.proj = nn.Conv2d(features, 1, kernel_size=1, bias=False)
     
        self.mgno = nn.ModuleList()
        for l in range(2):
            self.mgno.append(MG_FEM(in_channels=features, out_channels=features, num_iterations=[1,1,1,1,1,1,]))
...
def objective(dataOpt, modelOpt, optimizerScheduler_args,
    ...
    train_loader = DataLoader(TensorDataset(u[0:2000], f[0:2000], a[0:2000]), batch_size=dataOpt['batch_size'], shuffle=True)
    test_loader = DataLoader(TensorDataset(u[2000:2200], f[2000:2200], a[2000:2200]), batch_size=dataOpt['batch_size'], shuffle=False)
    ...
    # model = torch.load('/home/liux0t/FMM/MgNO/model/MgNO_DCdarcy_ufa_42024-09-01 20:29:50.048537.pt')
    # model = torch.load('/home/liux0t/FMM/MgNO/model/MgNO_DCdarcy_ufa_42024-09-01 20:53:43.310245.pt')
    # model = torch.load('/home/liux0t/FMM/MgNO/model/MgNO_DCdarcy_ufa_42024-09-01 21:55:49.901850.pt')
    model = torch.load('/home/liux0t/FMM/MgNO/model/MgNO_DCdarcy_ufa_42024-09-01 22:17:57.985740.pt')
    if log_if:    
...
if __name__ == "__main__":
    ...
    darcy_ufa_copy.objective(dataOpt, modelOpt, optimizerScheduler_args, model_type=args['model_type'],
